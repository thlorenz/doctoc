...
function transformAndSave(files, mode, maxHeaderLevel, title, notitle, asteriskFlag) {
  console.log('\n==================\n');
  
  var transformed = files
    .map(function (x) {
      var content = fs.readFileSync(x.path, 'utf8')
        , result = transform(content, mode, maxHeaderLevel, title, notitle, asteriskFlag);
      result.path = x.path;
      return result;
    });
...
function printUsageAndExit(isErr) {
  ...
  outputFunc('Usage: doctoc [mode] [--asterisk] [--notitle | --title title] [--maxlevel level] <path> (where path is some path to a directory (e.g., .) or a file (e.g., README.md))');
  outputFunc('\nAvailable modes are:');
...
var argv = minimist(process.argv.slice(2)
    , { boolean: [ 'h', 'help', 'T', 'notitle', 'a', 'asterisk' ].concat(Object.keys(modes))
...
var asteriskFlag = argv.a || argv.asterisk
...
for (var i = 0; i < argv._.length; i++) {
  ...
  transformAndSave(files, mode, maxHeaderLevel, title, notitle, asteriskFlag);
