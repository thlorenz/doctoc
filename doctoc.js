...
function transformAndSave(files, mode, maxHeaderLevel, title, notitle, editMessage, entryPrefix, processAll, stdOut, updateOnly) {
  if (processAll) {
  ...
  var transformed = files
    .map(function (x) {
      var content = fs.readFileSync(x.path, 'utf8')
        , result = transform(content, mode, maxHeaderLevel, title, notitle, editMessage, entryPrefix, processAll, updateOnly);
      result.path = x.path;
      return result;
    });
...
function printUsageAndExit(isErr) {
  ...
  outputFunc('Usage: doctoc [mode] [--entryprefix prefix] [--notitle | --title title] [--edit-message message] [--maxlevel level] [--all] [--update-only] <path> (where path is some path to a directory (e.g., .) or a file (e.g., README.md))');
  outputFunc('\nAvailable modes are:');
...
var argv = minimist(process.argv.slice(2)
    ...
    , string: [ 'title', 't', 'maxlevel', 'm', 'entryprefix', 'edit-message' ]
...
var editMessage = '<!-- ' + (argv['edit-message'] ? argv['edit-message'] : "DON'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE") + ' -->'
...
for (var i = 0; i < argv._.length; i++) {
  ...
  transformAndSave(files, mode, maxHeaderLevel, title, notitle, editMessage, entryPrefix, processAll, stdOut, updateOnly);
