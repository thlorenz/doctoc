...
function rankify(headers, max, min) {
  return headers
    .map(function (x) {
      x.rank = parseInt(x.tagName.slice(1), 10);
      return x;
    })
    .filter(function (x) {
...
var go = (module.exports = function (lines, maxHeaderLevel, minHeaderLevel) {
  var source = md
    ...
    .join("\n");
  var headers = parse(source, maxHeaderLevel, minHeaderLevel);
  headers = addLinenos(lines, headers);
  return headers;
};

module.exports.parse function parse(source, maxHeaderLevel, minHeaderLevel) {
  ...
  var parser = new htmlparser.Parser(
    {
      ...
      onclosetag: function (name) {
        ...
        if (grabbing[grabbing.length - 1].tagName === name) {
          ...
          headers.push({ text: text, tagName: tag.tagName, href: tag.tagId });
          text = [];
        }
      },
    },
