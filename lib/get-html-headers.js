  ...
  var parser = new htmlparser.Parser(
    {
      onopentag: function (name, attr) {
        ...
        if (grabbing[grabbing.length - 1]?.tag === "pre") return;

        if (name === "pre" || /h\d/.test(name)) {
          grabbing.push({ tag: name, line: currentLine });
        }
      },
      ontext: function (text_) {
        ...
        if (grabbing.length === 0 || grabbing[grabbing.length - 1].tag === "pre")
          return;
      ...
      onclosetag: function (name) {
        ...
        if (grabbing[grabbing.length - 1].tag === name) {
          var tag = grabbing.pop();
          headers.push({ text: text, tag: tag.tag });
          text = [];
        }
      },
    },
