...
function getMarkdownHeaders (lines, start, maxHeaderLevel, minHeaderLevel) {
  ...
  function isHtmlHeader (html, minHeaderLevel) {
    if (html[0] !== '<' || html[1] !== 'h') return false;
    const level = html[2];
    if (level !== '1' && level !== '2') return false;
    if (Number(level) < minHeaderLevel) return false;
    if (Number(level) > (maxHeaderLevel || 4)) return false;
    if (!html.endsWith('</h' + level + '>')) return false;
    return html.indexOf('<h', 3) === -1;
  }
  ...
  return lines
    ...
    .map(function (x) {
      if ((x.type === md.Syntax.Html || x.type === md.Syntax.HtmlBlock) && isHtmlHeader(x.raw, minHeaderLevel)) {
        return { node: x };
      }
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  ...
  options.toc.items = options.toc.items || {};
  options.toc.items.indentation = options.toc.items.indentation || {};

  syntax = syntax || 'md';
  if (options.toc.items.indentation.style == null) options.toc.items.indentation.style = 'space';
  if (options.toc.items.indentation.width == null) options.toc.items.indentation.width = (mode === 'bitbucket.org' || mode === 'gitlab.com') ? 4 : 2;
  if (options.toc.items.symbols == null) options.toc.items.symbols = String(entryPrefix || '-').split(',');
