...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  ...
  var initialNode = docNodes[startPos > 0 ? 1 : 0]
  if (!tocPosition.existing && options?.toc?.position == 'After' && initialNode.loc.start.line == initialNode.loc.end.line &&
      (
       (initialNode.type == md.Syntax.Heading && initialNode.raw.startsWith('# ')) ||
       (initialNode.type == md.Syntax.Html && initialNode.raw.startsWith('<h1') && initialNode.raw.endsWith('</h1>') && initialNode.raw.split('>').length == 3)
      )
     ) {
    var insertPos = docNodes[startPos > 0 ? 2 : 1].range[0];
