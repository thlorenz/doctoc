...
function generateComments(syntax){
  var start = generateCommentedBlock(syntax, "START doctoc generated TOC please keep comment here to allow auto update") + "\n" + 
    generateCommentedBlock(syntax, "DON\'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE");
  var end = generateCommentedBlock(syntax, "END doctoc generated TOC please keep comment here to allow auto update");
...
function generateCommentedBlock(syntax, content) {
  var commentStart = commentTypes[syntax].start
  , commentEnd = commentTypes[syntax].end;
  return `${commentStart} ${content} ${commentEnd}`;
}

function generateMarkers(lines, info, header, footer, syntax){
  var start = "", end = "";

  if (header && info.hasStart) {
    start = lines[info.startIdx];
    start = start.replace(" generated TOC please keep comment here to allow auto update", "");
    console.log("start: ", start);
  }
  else if (!header) {
    start = generateCommentedBlock(syntax, "START doctoc generated TOC please keep comment here to allow auto update");
  }
  else {
    start = generateCommentedBlock(syntax, "START doctoc");
  }

  if (footer && info.hasStart) {
    end = lines[info.endIdx];
    end = end.replace(" generated TOC please keep comment here to allow auto update", "");
  }
  else if (!footer) {
    end = generateCommentedBlock(syntax, "END doctoc generated TOC please keep comment here to allow auto update");
  }
  else {
    end = generateCommentedBlock(syntax, "END doctoc");
  }
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, padTitle, options) {
  if (options !== undefined) {
    console.log("options", options);
  }

  syntax = syntax || "md"
  generateComments(syntax)
  var skipTag = generateCommentedBlock(syntax, "DOCTOC SKIP") + '\n';
  ...
  var { start, end } = generateMarkers(lines, info, options?.toc?.header?.remove, options?.toc?.footer?.remove, syntax);

  if (options?.toc?.header?.content) {
    start = start + '\n' + options.toc.header.content;
  }
  else if (options?.toc?.header?.remove != true) {
    start = start + '\n' + generateCommentedBlock(syntax, "DON\'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE");
  }
