...
module.exports = function transform(content, mode) {
  ...
  var lines;
  if (mode === 'plain') {
      // Clear anchors in headers first for the case transformed before
      lines = content.split('\n').map(function(line) {
        if (/^(\#{1,8})[ ]*(.+)/.test(line)) {
  ...
  if (mode === 'plain') {
      ...
      linkedHeaders.forEach(function(linkedHeader) {
        var header = lines[linkedHeader.line];
        var anchor = linkedHeader.anchor.match(/.*\(#(.+)\)/)[1];
        var link = '<a id="' + anchor + '"></a>';
        lines[linkedHeader.line] = header.replace(linkedHeader.name, link + linkedHeader.name);
      });
  }
