...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  ...
  var startLine = info.hasEnd ? info.endIdx : info.startIdx;

  var { start, end } = generateMarkers(lines, info, options?.toc?.pragma?.style, syntax);
  var headers = getMarkdownHeaders(content, maxHeaderLevel, minHeaderLevel)
    .concat(getHtmlHeaders(content, maxHeaderLevelHtml, minHeaderLevel))
    .filter(function (x) {
      return processAll || !info.hasStart || x.line >= startLine + 1;
    });
