...
function processHeaders(headers, mode) {
  ...
  for (var i = 0; i < headers.length; i++) {
    ...
    var anchor = anchor(header.name, mode, 0, header.href);
    var name = header.href || anchor.split('#')[1].slice(0, -1);

    if (!linkDefined && Object.prototype.hasOwnProperty.call(instances, name)) {
    ...
    header.anchor = instances[name] > 0 ? anchor(header.name, mode, instances[name], header.href) : anchor;
  }
