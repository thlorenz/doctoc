...
function getMarkdownHeaders (lines, maxHeaderLevel, noh1) {
  ...
  return md.parse(lines.join('\n')).children
    ...
    .map(function (x) {
      return !(noh1 && x.depth == 1) && (!maxHeaderLevel || x.depth <= maxHeaderLevel)
        ? { rank :  x.depth
          , name :  extractText(x)
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, title, notitle, noh1, entryPrefix, processAll, updateOnly) {
  ...
  var headers = getMarkdownHeaders(linesToToc, maxHeaderLevel, noh1)
    .concat(getHtmlHeaders(linesToToc, maxHeaderLevelHtml, noh1))
