...
function getMarkdownHeaders (lines, maxHeaderLevel, minHeaderLevel) {
  function extractText (header) {
    return header.children === undefined ? header.raw : header.children
      .map(function (x) {
  ...
  return md
    ...
    .children.filter(function (x) {
      return x.type === md.Syntax.Header || x.type === md.Syntax.HtmlBlock || x.type === md.Syntax.Html;
    })
    .map(function (x) {
      if (x.type === md.Syntax.Header) {
        return {
          rank :  x.depth,
          name :  extractText(x),
      ...
      else if (x.type === md.Syntax.HtmlBlock || x.type === md.Syntax.Html){
        var headers = getHtmlHeaders.parse(extractText(x), maxHeaderLevel, minHeaderLevel);
        if (headers.length != 1) { return null; }
        headers[0].line = x.loc.start.line;
        return headers[0];
      }
      else {
        return null;
...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  ...
  var headers = getMarkdownHeaders(linesToToc, maxHeaderLevel, minHeaderLevel)
    .filter(function (x) {
      return x.rank >= minHeaderLevel && (!maxHeaderLevel || x.rank <= maxHeaderLevel);
    });
  ...
  if (allHeaders.length >= minTocItems) {
    toc = 
      titlePadding
    + inferredTitle
    + titleSeparator
