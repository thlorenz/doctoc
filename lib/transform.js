...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  ...
  if (options?.toc?.items === undefined && (entryPrefix === 'number' || entryPrefix === 'uppercase' || entryPrefix === 'lowercase' || entryPrefix === 'roman')) {
    options.toc.items.style = entryPrefix;
    options.toc.items.format = 'ordered';
    entryPrefix = '';
  }
  else if (options?.toc?.items === undefined) {
    options.toc.items.format = 'unordered';
    options.toc.items.symbol = entryPrefix;
  }
  ...
  if (headers.length >= minTocItems && docNodes.children[docNodes.children.length - 1].loc.end.line >= minLines) {
    ...
    tocLines.push(...allHeaders
        .map(function (x) {
          ...
          if (options.toc.items.style === 'number') {
            symbol = count.join('.') + '.';
          }
          else if (options.toc.items.style === 'lowercase' || options.toc.items.style === 'uppercase') {
            let columnName = '';
          ...
          else if (options.toc.items.style === 'roman') {
