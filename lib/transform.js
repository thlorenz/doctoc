...
exports = module.exports = function transform(content, options) {
  options.mode = options.mode || 'github.com';
  options.entryPrefix = options.entryPrefix || '-';

  // only limit *HTML* headings by default
  var maxHeaderLevelHtml = options.maxHeaderLevel || 4;

  var lines = content.split('\n')
    , info = updateSection.parse(lines, matchesStart, matchesEnd)

  var inferredTitle = determineTitle(options.title, options.notitle, lines, info);

  var currentToc = info.hasStart && lines.slice(info.startIdx, info.endIdx + 1).join('\n')
    , linesToToc = getLinesToToc(lines, currentToc, info);

  var headers = getMarkdownHeaders(linesToToc, options.maxHeaderLevel)
  ...
  var allHeaders    =  countHeaders(headers)
    ...
    , linkedHeaders =  _(allHeaders).map(addAnchor.bind(null, options.mode));

  if (linkedHeaders.length === 0) return { transformed: false };

  // 4 spaces required for proper indention on Bitbucket and GitLab
  var indentation = (options.mode === 'bitbucket.org' || options.mode === 'gitlab.com') ? '    ' : '  ';
  ...
  var toc =
    ...
    + linkedHeaders
        ...
        return indent + options.entryPrefix + ' ' + x.anchor;
        })
        .join('\n')
    + '\n';
