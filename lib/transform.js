...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, title, notitle, entryPrefix, processAll, updateOnly) {
  ...
  // https://github.com/thlorenz/doctoc/issues/174
  if (mode === "bitbucket.org") {
    var newlinesToToc = [];
    for (var i = 0; i < linesToToc.length; i++) {
      var mappedHeader = linkedHeaders.find((x) => i === x.line - 1);
      if (mappedHeader) {
        var headerAnchor = mappedHeader.anchor.split("#")[1].replace(")", "");
        var expectedHtmlAnchor = '<a name="' + headerAnchor + '"></a>';

        if (expectedHtmlAnchor !== linesToToc[i - 1]) {
          newlinesToToc.push(expectedHtmlAnchor);
        }
      }
      newlinesToToc.push(linesToToc[i]);
    }
    data = updateSection(
      newlinesToToc.join("\n"),
      wrappedToc,
