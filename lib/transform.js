...
exports = module.exports = function transform(content, mode, maxHeaderLevel, minHeaderLevel, minTocItems, title, notitle, entryPrefix, processAll, updateOnly, syntax, options) {
  ...
  if (hasBody && docNodes.children[0].type === 'Yaml') {
    frontMatter = true;
  }
  else if(hasBody && docNodes.children[0].type === 'Paragraph' && docNodes.children[0].raw.startsWith('+++') && docNodes.children[0].raw.trim().endsWith('+++')) {
    frontMatter = true;
  }
  else if(hasBody && docNodes.children[0].type === 'Paragraph' && docNodes.children[0].raw.startsWith(';;;') && docNodes.children[0].raw.trim().endsWith(';;;')) {
    frontMatter = true;
  ...
  // force it on to the next line with a line between
  else if (frontMatter) {
    startPos = docNodes.children[0].range[1] + 2;
    hasBody = false;
  }
  // add additional line when doc has no trailing
