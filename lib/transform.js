...
function basicGithubIdWithPageInAnchor(page, anchor) {
  var githubStyledPageName = page.replace(/ /g,'-').replace(/[\/?:\[\]`,()*"]/g,'');
  return anchor.replace(/\#/g,githubStyledPageName+'\#')
}
    ...
    + linkedHeaders
        .map(function (x) {
          var indent = _(_.range(x.rank - lowestRank))
            ...
            if (isWiki) {
              for (var i = linkedHeaders.length - 1; i >= 0; i--) {
                if(linkedHeaders[i].rank == 1){
                  x.anchor = basicGithubIdWithPageInAnchor(linkedHeaders[i].name.toLowerCase(),x.anchor);
                }
              };
            }
